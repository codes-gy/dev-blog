                ...
                <div className="mt-8 flex flex-col gap-10 md:mt-14 md:gap-16">
                    <section className="relative border-l-2 border-slate-200 pl-5 md:pl-7 dark:border-slate-800">
                        ...
                        <ProjectSection
                            ...
                            troubleshooting={{
                                situation:
                                    '지하주차장 또는 주변에 건물이 없는 장소에서 사고 접수를 하면 기사님에게 이상한 위치 좌표가 전달되는 문제가 있었습니다.',
                                task: '현장 기사님에게 전달되는 좌표의 오차 범위를 줄여야 했습니다.',
                                action: '기기의 GPS 정보 외에도 주변 기지국/Wi-Fi 기반의 위치 보정 파라미터까지 같이 받아서, 백엔드에서 삼각측량 기반 보정 알고리즘을 거치도록 재설계했습니다.',
                                result: '음영 지역에서의 좌표 오차 범위를 200m 이내로 축소했습니다.',
                                takeaway: '수집된 데이터의 제한을 백엔드에서 보정하여 해결하는 경험을 했습니다.',
                            }}
                        />
                    </section>
                    ...
                    <section className="relative border-l-2 border-slate-200 pl-5 md:pl-7 dark:border-slate-800">
                        ...
                        <ProjectSection
                            ...
                            troubleshooting={{
                                situation:
                                    "서비스 전체 알림이 포화 상태일때, '서버 장애 긴급 알림'이 몇 분 뒤에 발송되는 문제를 확인했습니다.",
                                task: '중요한 알림이 밀리지 않고 즉시 발송되도록 우선순위를 지정이 필요했습니다.',
                                action: 'RabbitMQ Priority Queue를 적용해서 알림 메시지에 우선순위 등급을 매기고, 긴급 알림 전용 스레드를 따로 배정했습니다.',
                                result: '긴급 장애 알림 전달 지연을 10초 이내로 줄였습니다.',
                                takeaway: '작업 특성에 따라 큐 채널을 분리하고 우선순위를 두는 경험을 했습니다.',
                            }}
                        />
                    </section>
                    ...
                    <section className="relative border-l-2 border-slate-200 pl-5 md:pl-7 dark:border-slate-800">
                        ...
                        <ProjectSection
                            ...
                            troubleshooting={{
                                situation:
                                    '관리자가 메인 배너나 콘텐츠를 잘못 입력해 배포했을 때, 이전 상태로 되돌리는 기능이 없어 일일이 다시 수기 입력해야 했습니다.',
                                task: '오배포 시 버튼 하나로 이전 버전으로 롤백하는 시스템이 필요했습니다',
                                action: '테이블에 버전관리 스키마를 설계하고, 클릭 한 번으로 과거 버전 스냅샷을 최신버전으로 변경하는 롤백 API를 만들었습니다.',
                                result: '콘텐츠 배포 실수 시 복구 시간을 30분에서 단 10초로 줄였습니다.',
                                takeaway:
                                    '운영 도구를 만들 때는 오배포 상황을 대비한 버전 관리와 롤백을 꼭 염두에 두어야 함을 깨달았습니다.',
                            }}
                        />
                    </section>
                    ...
                    <section className="relative border-l-2 border-slate-200 pl-5 md:pl-7 dark:border-slate-800">
                        ...
                        <ProjectSection
                            ...
                            troubleshooting={{
                                situation:
                                    '교육청 지침이 바뀌어 문서 서식이 달라질 때마다, 백엔드의 PDF 생성 Java 코드를 일일이 수정하고 배포해야 했습니다.',
                                task: '서식이 바뀌어도 Java 코드 수정 없이 유연하게 대응하는 구조가 필요했습니다.',
                                action: 'HTML 기반의 템플릿(Thymeleaf)을 사용하고, 백엔드는 데이터만 넘겨주면 HTML을 PDF로 변환해 주는 Decoupled 구조로 변경했습니다.',
                                result: '서식이 바뀌어도 Java 코드 배포 없이 HTML 템플릿만 수정해서 즉시 대응할 수 있게 되었습니다.',
                                takeaway: '파일 생성 로직과 서식 레이아웃을 분리해 두는 패턴을 경험했습니다',
                            }}
                        />
                    </section>
                </div>
